...
import re
import base64
from app_reviews_workshop import init_app_crew
...
"""
st.markdown(f'<h1 style="display: flex; align-items: center;"><span style="font-size: 32px; margin-right: 10px;">{robot_icon}</span> App Store Review Analysis</h1>', unsafe_allow_html=True)

app_name = st.text_input("Enter app name", value="minecraft",disabled=st.session_state['processing'])
# ranks_options = ['ALL', '5', '4', '3','2','1']
ranks_options = {'All':'-1', '5':'5', '4':'4', '3':'3','2':'2','1':'1'}
selected_rank_label = st.selectbox('Select Ranks', ranks_options,disabled=st.session_state['processing'])
selected_rank = ranks_options[selected_rank_label]
country_options = {"USA":"us", "Singapore":"sg", "Japanese":"jp","Korea":"kr"}
selected_country_label =st.selectbox('Select Country', country_options)
selected_country = country_options[selected_country_label]

if st.button("Click This Button to Trigger Analyze", type="primary", use_container_width=True):
    st.info(f"debug: app_name: {app_name}, selected_rank: {selected_rank}, selected_country: {selected_country}")
    expander = st.expander("Processing...", expanded=True)
    with expander:
        stream_to_expander = StreamToExpander(expander)    
        app_id,crew = init_app_crew( selected_country_label,app_name,selected_country,selected_rank,page=1,output_stream=stream_to_expander)
    ...
    with analysis_complete_placeholder:
        ...
        with st.expander("Show Analysis Result", expanded=True) :
